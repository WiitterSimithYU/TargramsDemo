...
//    }
    ...
    if (!testLabel) {
        ...
        testLabel.frame =CGRectMake(2, 2, 150,50);
    ...
    if ([layout isKindOfClass:[TangramFixLayout class]]) {
        NSString *cellClassString = NSStringFromClass([TangramFixLayout class]);
        testLabel.text = [NSString stringWithFormat:@"%@",cellClassString];
    }else if ([layout isKindOfClass:[TangramStickyLayout class]]){
        NSString *cellClassString = NSStringFromClass([TangramStickyLayout class]);
        testLabel.text = [NSString stringWithFormat:@"%@",cellClassString];
    }else if ([layout isKindOfClass:[TangramSingleAndDoubleLayout class]]){
        NSString *cellClassString = NSStringFromClass([TangramSingleAndDoubleLayout class]);
        testLabel.text = [NSString stringWithFormat:@"%@",cellClassString];
    }else if ([layout isKindOfClass:[TangramWaterFlowLayout class]]){
        NSString *cellClassString = NSStringFromClass([TangramWaterFlowLayout class]);
        testLabel.text = [NSString stringWithFormat:@"%@",cellClassString];
    }else if ([layout isKindOfClass:[TangramDragableLayout class]]){
        NSString *cellClassString = NSStringFromClass([TangramDragableLayout class]);
        testLabel.text = [NSString stringWithFormat:@"%@",cellClassString];
    }else{
...
- (UIView<TangramLayoutProtocol> *)layoutInTangramView:(TangramView *)view atIndex:(NSUInteger)index
{
    ...
    if (index == 2) {
        //DoubleColumn
        TangramDoubleColumnLayout *DoubleColumnLayout = [[TangramDoubleColumnLayout alloc] init];
    ...
    if (index == 4) {
        TangramPageScrollLayout *PageScrollLayout = [[TangramPageScrollLayout alloc] init];
        return PageScrollLayout;
    }
    if (index == 5) {
        TangramQuintetColumnLayout *QuintetColumnLayout = [[TangramQuintetColumnLayout alloc] init];
        return QuintetColumnLayout;
    }
    if (index == 6){
    ...
    layout.vGap = 5;
    layout.index = index;
    layout.backgroundColor = [UIColor purpleColor];
    return layout;
}
